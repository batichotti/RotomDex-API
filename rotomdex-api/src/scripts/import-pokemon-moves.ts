...
const PokemonMoveSchema = new EntitySchema({
	name: 'PokemonMove',
...
async function run() {
	...
	try {
		...
		const pokemonMoves = dadosCsv.map((row) => ({
		...
		await pokemonMoveRepository.save(pokemonMoves)

		console.log('✅ Importação de movimentos concluída com sucesso!')
	} catch (error) {
